...
def run(host, indexname, mov, output):
    ...
    y = []
    while end < max_t:
        count = max_txs(url, begin, end)
        if count is None:
            count = 0
        l.append((c * step, count))
        x.append(c * step)
        y.append(count)
...
def draw(img_file, csv_file):
    ...
    fig = go.Figure(
        data=go.Scatter(x=data["sequence"], y=data["txs"], mode="lines")
    )
    fig.update_layout()
